...
func (t *TestWSClient) New(_ string) (client.Client, error) {
	return t.client, nil
}
...
func TestAuthorized(t *testing.T) {
	...
	// Prepare Directive
	oc := fgamocks.NewOpenFGAServiceClient(t)
	...
	oc.EXPECT().ListStores(mock.Anything, mock.Anything).Return(listStoresResponse, nil)
	checkResponse := &openfgav1.CheckResponse{
		Allowed: true,
	}
	oc.EXPECT().Check(mock.Anything, mock.Anything).Return(checkResponse, nil)
	air := accountinfomocks.NewRetriever(t)
	air.EXPECT().Get(mock.Anything, "root:orgs:test").Return(&accountsv1alpha1.AccountInfo{
	...
	fakeClient := fake.NewClientBuilder().
		...
		Build()
	wsCLient := &TestWSClient{
		client: fakeClient,
	}
	ad := NewAuthorizedDirective(oc, air, 5*time.Minute, wsCLient, log)

	// Prepare WebToken in context
	...
	result, err := ad.Authorized(ctx, nil, next, "read")
...
//type mockWSClientFactory struct {
//	mock.Mock
//}
//
//func (m *mockWSClientFactory) New(accountPath string, log *logger.Logger) (client.Client, error) {
//	args := m.Called(accountPath, log)
//	if args.Get(0) == nil {
//		return nil, args.Error(1)
//	}
...
//}
//
//// Ensure mockWSClientFactory implements WorkspaceClient interface
//var _ WorkspaceClient = (*mockWSClientFactory)(nil)
//
//// Tests for WorkspaceClient
//func TestNewDefaultWSClientFactory(t *testing.T) {
//	restConfig := &rest.Config{Host: "https://test.example.com"}
//	scheme := runtime.NewScheme()
//
//	factory := NewDefaultWSClientFactory(restConfig, scheme)
//
//	assert.NotNil(t, factory)
//	assert.Equal(t, restConfig, factory.restCfg)
//	assert.Equal(t, scheme, factory.scheme)
//}
//
//func TestDefaultWSClientFactory_CreateClient(t *testing.T) {
//	log, _ := logger.New(logger.DefaultConfig())
//	accountPath := "test-account"
//
//	// Create a test REST config
...
//	factory := NewDefaultWSClientFactory(restConfig, scheme)
//
//	result, err := factory.New(accountPath, log)
//
//	assert.NoError(t, err)
//	assert.NotNil(t, result)
//}
//
//func TestDefaultWSClientFactory_CreateClient_InvalidHost(t *testing.T) {
//	log, _ := logger.New(logger.DefaultConfig())
//	accountPath := "test-account"
//
//	// Create a test REST config with invalid host
...
//	assert.NotNil(t, directive.wcClient)
//	assert.NotNil(t, directive.helper)
//}
//
//func TestNewAuthorizedDirectiveWithFactory(t *testing.T) {
//	// Mock dependencies
//	mockClient := fgamocks.NewOpenFGAServiceClient(t)
//	mockAccountInfoRetriever := &mockAccountInfoRetriever{}
//	mockClientFactory := &mockWSClientFactory{}
//	storeTTL := 5 * time.Minute
//
//	directive := NewAuthorizedDirectiveWithFactory(mockClient, mockAccountInfoRetriever, storeTTL, mockClientFactory)
//
//	assert.NotNil(t, directive)
//	assert.Equal(t, mockClient, directive.fga)
//	assert.Equal(t, mockAccountInfoRetriever, directive.air)
//	assert.Equal(t, mockClientFactory, directive.wcClient)
//	assert.NotNil(t, directive.helper)
//}
//
...
//		fga:           mockClient,
//		helper:        mockHelper,
//		wcClient: &mockWSClientFactory{}, // Not used in this test
//	}
//
//	// Create test data
//	ctx := context.Background()
...
//}
//
//// Test demonstrating mocked WorkspaceClient functionality
//func TestAuthorizedDirective_WithMockedWSClientFactory(t *testing.T) {
//	// Create mocks
//	mockFGAClient := fgamocks.NewOpenFGAServiceClient(t)
//	mockAccountInfoRetriever := &mockAccountInfoRetriever{}
//	mockHelper := &mockStoreHelper{}
//	mockClientFactory := &mockWSClientFactory{}
//
//	// Create a fake Kubernetes client for testing resource existence
//	scheme := runtime.NewScheme()
//	err := accountsv1alpha1.AddToScheme(scheme)
//	require.NoError(t, err)
//
...
//		WithScheme(scheme).
//		WithObjects(namespacedAI).
//		Build() // Not used due to JWT token validation failure
//
//	// Create directive with all mocked dependencies
//	directive := &AuthorizedDirective{
//		fga:           mockFGAClient,
//		helper:        mockHelper,
//		air:           mockAccountInfoRetriever,
//		wcClient: mockClientFactory,
//	}
//
//	// Setup test context
//	ctx := context.Background()
...
//
//	// Create test data
//	_ = createTestWebToken() // Token not used in this specific test
//	kcpCtx := appcontext.KCPContext{
//		IDMTenant:        "test-tenant",
//		OrganizationName: "test-org", // Matches ai.Spec.Organization.Name
//	}
//	ctx = appcontext.SetKCPContext(ctx, kcpCtx)
//	ctx = pmcontext.AddWebTokenToContext(ctx, "valid.jwt.token", []jose.SignatureAlgorithm{jose.RS256})
//
//	fieldCtx := &graphql.FieldContext{
//		Args: map[string]any{
//			"context": map[string]any{
...
//	ctx = graphql.WithFieldContext(ctx, fieldCtx)
//
//	// Mock expectations - these won't be called due to JWT token failure, but they show how to mock the WorkspaceClient
//	// mockAccountInfoRetriever.On("Get", mock.Anything, "test-account").Return(ai, nil)
//	// mockClientFactory.On("New", "test-account", mock.Anything).Return(fakeK8sClient, nil)
//	// mockHelper.On("GetStoreID", mock.Anything, mockFGAClient, "test-org").Return("test-store-id", nil)
//	// mockFGAClient.EXPECT().Check(mock.Anything, mock.Anything).Return(&openfgav1.CheckResponse{Allowed: true}, nil)
//
//	// Mock next resolver
//	nextCalled := false
//	next := func(ctx context.Context) (any, error) {
...
//	}
//
//	// Execute test - this will fail due to JWT token validation but demonstrates the mock setup
//	result, err := directive.Authorized(ctx, nil, next, "read")
//
//	// The test will likely fail at JWT token validation, but this demonstrates
//	// how the WorkspaceClient can now be mocked for more comprehensive testing
//	if err != nil {
//		// Expected to fail at JWT token validation in test environment
//		assert.Contains(t, err.Error(), "failed to get web token from context")
//		assert.Nil(t, result)
//	} else {
//		// If it somehow passes JWT validation, verify the success path
//		assert.Equal(t, "success", result)
//		assert.True(t, nextCalled)
//	}
//
//	// Since the test fails at JWT validation, we don't need to assert mock expectations
//	// In a real test scenario with proper JWT setup, you would assert:
//	// mockAccountInfoRetriever.AssertExpectations(t)
