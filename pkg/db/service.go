...
func New(cfg ConfigDatabase, dbConn *gorm.DB, logger *logger.Logger, migrate bool, isLocal bool) (*Database, error) {
	...
	if isLocal { // nolint: nestif
		...
		if err != nil {
			logger.Error().Err(err).Str("file path", cfg.LocalData.DataPathUser).Msg("failed to load user data")
		}
		err = database.LoadInvitationData(cfg.LocalData.DataPathInvitations)
		if err != nil {
			logger.Error().Err(err).Str("file path", cfg.LocalData.DataPathInvitations).Msg("failed to load invitation data")
		}
		err = database.LoadTeamData(cfg.LocalData.DataPathTeam, users)
		if err != nil {
			logger.Error().Err(err).Str("file path", cfg.LocalData.DataPathTeam).Msg("failed to load team data")
		}
		err = database.LoadTenantConfigData(cfg.LocalData.DataPathTenantConfiguration)
		if err != nil {
			logger.Error().Err(err).Str("file path", cfg.LocalData.DataPathTenantConfiguration).Msg("failed to load tenant config data")
		}
		err = database.LoadRoleData(cfg.LocalData.DataPathRoles)
		if err != nil {
			logger.Error().Err(err).Str("file path", cfg.LocalData.DataPathRoles).Msg("failed to load role data")
		}
	}
